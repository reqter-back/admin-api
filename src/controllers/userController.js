...
exports.register = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      console.log(req.body);
      broker
        .sendRPCMessage({ body: req.body }, "adminregister")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          console.log(obj);
          if (!obj.success) {
...
exports.changeavatar = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      broker
        .sendRPCMessage({ body: req.body }, "adminchangeavatar")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
...
exports.changenotification = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      req.body.id = req.userId;
      broker
        .sendRPCMessage({ body: req.body }, "adminchangenotification")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
...
exports.updateprofile = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      broker
        .sendRPCMessage(
          { spaceId: req.body.spaceId, userId: req.userId, body: req.body },
          "adminupdateprofile"
        )
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
...
exports.findbyemail = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      console.log("add system user started.");
      broker
        .sendRPCMessage({ body: req.body }, "adminfindbyemail")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
...
exports.getuserinfo = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      broker
        .sendRPCMessage({ body: { id: req.userId } }, "getadminuserinfo")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
...
exports.forgotpassword = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      req.body.id = req.userId;
      broker
        .sendRPCMessage({ body: req.body }, "admingetforgotpasswordtoken")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
...
exports.confirmemail = [
  (req, res, next) => {
    console.log(req);
    broker
      .sendRPCMessage({ body: { id: req.userId } }, "adminconfirmemail")
      .then(result => {
        var obj = JSON.parse(result.toString("utf8"));
        if (!obj.success) {
          if (obj.error) {
...
exports.resetpassword = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      broker
        .sendRPCMessage(
          { body: { id: req.userId, newpassword: req.body.newpassword } },
          "adminresetpassword"
        )
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
...
exports.changepassword = [
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      req.body.id = req.userId;
      broker
        .sendRPCMessage({ body: req.body }, "adminchangepassword")
        .then(result => {
          var obj = JSON.parse(result.toString("utf8"));
          if (!obj.success) {
            if (obj.error) {
